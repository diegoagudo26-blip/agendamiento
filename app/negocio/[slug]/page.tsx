...
export default function NegocioPage() {
  ...
  useEffect(() => {
    const cargar = async () => {
      ...
      setCitasOcupadas((citasData || []).map(c => c.fecha_hora.slice(0, 16)))
    }
    if (slug) cargar()
  }, [slug])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('citas').insert([{
      ...form, servicio_id: servicioId, fecha_hora,
      negocio_id: negocio.id, estado: 'pendiente'
    }])
    if (!error && servicio) {
      await fetch('/api/enviar-email', {
        ...
        body: JSON.stringify({
          ...
          notas: form.notas,
          negocio_email: negocio ? (negocio as any).email : null
        })
      })
    }
    setCargando(false)
  ...
  if (noEncontrado) return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-10 text-center">
        ...
        <h2 className="text-xl font-bold text-gray-700 mt-2">Negocio no encontrado</h2>
      </div>
    </main>
  )
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-8">
      <div className="bg-white rounded-2xl shadow-xl p-10 max-w-lg w-full">
        ...
        <p className="text-gray-500 text-sm mb-6">Reserva tu cita en línea</p>
